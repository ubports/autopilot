...
class Eventually(Matcher):
    ...
    2. Since the test is running in a separate process to the application under
       test, test authors cannot make any assumptions about when the application
       under test will recieve CPU time to update to the expected state.
    ...
    **Attributes from the application**::
    ...
    an autopilot test. Note that any matcher can be used - either from testtools
    or any custom matcher that implements the matcher API::
    ...
    **Callable Objects**::
    ...
    In this example we're using the :func:`autopilot.platform.model` function as
    a callable. In this form, Eventually matches against the return value of the
    callable.
    ...
    .. note:: Using this form generally makes your tests less readabvle, and
        ...
        In this example, if ``self.mouse.x`` were ever to change to be a regular
        python attribute, this test would likely break.

    **Timeout**

    By default timeout period is ten seconds. This can be altered by passing the
    timeout keyword::
